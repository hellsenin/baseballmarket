...
<%
 /**
  * @Class Name  : DetailCodeRegist.jsp
  * @Description : DetailCodeRegist 화면
...
<CAPTION style="display: none;">공통상세코드 등록</CAPTION>
  <tr>
    ...
    <td width="80%" nowrap colspan="3">
		<select name="clCode" class="select" onChange="javascript:fn_egov_get_CodeId(document.detailCode);" title="clCode">
			<c:forEach var="result" items="${clCodeList}" varStatus="status">
				<option value='<c:out value="${result.clCode}"/>' <c:if test="${result.clCode == code.clCode}">selected="selected"</c:if>><c:out value="${result.clCodeNm}"/></option>
			</c:forEach>
		</select>
...
-->  
  ...
  <td width="10"></td>
  <td><span class="button"><input type="submit" value="저장" onclick="fn_egov_regist_CmmnDetailCode(document.detailCode); return false;"></span></td>
</tr>
</table>
