package com.bbm.util.sys.dbm.web;
...
import com.bbm.util.sys.dbm.service.DbMntrng;
import com.bbm.util.sys.dbm.service.DbMntrngLog;
import com.bbm.util.sys.dbm.service.EgovDbMntrngService;
...
public class EgovDbMntrngController {
	...
	public String insertDbMntrng(DbMntrng dbMntrng, BindingResult bindingResult, ModelMap model)
	  throws Exception{
    	...
    	if (bindingResult.hasErrors()){
    		...
    		//아이디 설정
			dbMntrng.setLastUpdusrId((String)loginVO.getUniqId());
			dbMntrng.setFrstRegisterId((String)loginVO.getUniqId());
	...
	private void referenceData(ModelMap model) throws Exception {
        ...
        vo.setCodeId("COM048");
        List<DetailCode> dbmsKindList = cmmUseService.selectCmmCodeDetail(vo);
	...
	private void checkDuplication(DbMntrng obj, Errors errors) {
		DbMntrng dbMntrng = (DbMntrng) obj;
